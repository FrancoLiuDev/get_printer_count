...
- 📊 批量擷取印表機使用統計
- 📁 支援 Excel 輸入檔案
- � scan_code 篩選功能，可針對特定設備進行查詢
- 🚀 智能端口檢測（22/443），快速跳過無法連線的設備
- �💾 輸出 CSV 格式報表
- � 內建除錯模式
...
**注意：** 較新型號如 HP LaserJet E50145 等使用不同的介面協定，目前尚未支援自動擷取。如需支援請聯繫技術人員。
...
python3 GetPrinterCount.py --excel printers.xlsx --scan_code A001 --out output.csv
```
...
可選參數：
  ...
  --scan_code SCAN_CODE 根據 scan_code 欄位篩選特定設備
...
GetPrinterCount.exe --excel printers.xlsx --scan_code A001 --out output.csv
```
...
#### 4. 根據 scan_code 篩選特定設備

```cmd
GetPrinterCount.exe --excel printers.xlsx --scan_code A001 --out A001_report.csv
```

> 只處理 Excel 中 scan_code 欄位為 "A001" 的印表機

#### 5. 組合使用多個參數

```cmd
GetPrinterCount.exe --excel printers.xlsx --scan_code B002 --out B002_report.csv --debug
```

> 篩選 scan_code 為 "B002" 的設備並啟用除錯模式
...
- `ok` - 成功擷取資料
- `port_closed` - 無法連線（22 和 443 端口都無法連接）
- `no_xml` - 無法取得 XML 資料或連線逾時
- `unknown_model` - 不支援的印表機型號
- `parse_error:[錯誤類型]` - XML 解析錯誤
...
**狀態訊息：** `port_closed` - 22 和 443 端口都無法連線
**解決方法：**

- 檢查印表機 IP 位址是否正確
- 確認印表機已開機且連接網路
- 檢查防火牆設定，確保端口 22 或 443 可連線
- 嘗試 ping 印表機 IP
- 程式會自動跳過無法連線的設備，加快處理速度
...
- ✨ 新增 scan_code 篩選功能
- ⚡ 新增智能端口檢測（22/443），大幅提升處理速度
- 🐛 改善錯誤處理機制
- 📊 新增更詳細的狀態回報（ok, port_closed, no_xml, unknown_model, parse_error）
- 🔧 優化連線超時設定（3-5秒）
